self.onmessage = function(event) {
    const { numSimulations, teamData, divisions, afcTeams, nfcTeams, playerMap } = event.data;

    // Initialize win probabilities for all players
    for (const playerData of Object.values(playerMap)) {
    ...
    for (let sim = 0; sim < numSimulations; sim++) {
        ...
        ['AFC', 'NFC'].forEach(conference => {
            ...
            const weights = remainingTeams.map(team => Math.max(0, team.probability_playoffs - team.probability_division_win));
        ...
        winners.forEach(winner => {
            playerMap[winner].winProbability += probPerWinner;
        });
    }

    // Send the updated playerMap back to the main thread
